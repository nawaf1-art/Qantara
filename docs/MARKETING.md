...
Current status: pre-1.0 Beta. The primary evaluation path is the browser voice UI + WebSocket pipeline + local STT/TTS + an OpenAI-compatible backend. MCP and the multi-device mesh are Experimental.
...
Current Beta surfaces: browser voice UI, local STT/TTS, barge-in, Docker/local setup, and the OpenAI-compatible backend path. MCP and the multi-device mesh are Experimental.
...
It works best today with local OpenAI-compatible backends such as Ollama, llama.cpp, LM Studio, Jan, LiteLLM, and vLLM. MCP integration is included as an experimental path.
