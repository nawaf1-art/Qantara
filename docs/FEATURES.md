...
| Barge-in and cancellation | Beta | Gateway enforces a bounded cancellation grace period when adapters do not cooperate. |
| faster-whisper STT | Beta | Local model dependency; first use may download model files. |
| Piper TTS | Beta | Local executable and voice files are operator-supplied outside the base wheel. |
...
| Mock adapter | Beta | Deterministic development and contract-test backend. |
...
The changelog is authoritative for shipped versions. The [roadmap](../ROADMAP.md) describes direction rather than availability.
