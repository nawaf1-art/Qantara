# Qantara Documentation

Current source and published release line: `0.4.0`.

Qantara `0.4.0` is the current source version. Its GitHub Release (wheel, source archive, checksums, SPDX SBOM, and validation evidence) is created only by the owner-controlled [release process](RELEASE_PROCESS.md); until the `v0.4.0` release is published, the latest published GitHub Release is `v0.3.1`. Qantara is not published to PyPI. The [documentation governance](DOCUMENTATION_GOVERNANCE.md) file defines authority, document classes, and the completeness gate.

## Start here

- [Quickstart](QUICKSTART.md)
- [Installation and first run](INSTALLATION_AND_FIRST_RUN_GUIDE.md)
- [CLI launcher](CLI.md)
- [Configuration](CONFIGURATION.md)
- [Troubleshooting](TROUBLESHOOTING.md)
- [FAQ](FAQ.md)

## Product and public surfaces

- [Feature matrix](FEATURES.md)
- [Use cases](USE_CASES.md)
- [Python SDK](PYTHON_SDK.md)
- [Voice API](VOICE_API.md)
- [Ollama compatibility](OLLAMA_COMPATIBILITY.md)
- [MCP](MCP.md)
- [Multi-device mesh](MESH.md)
- [Home Assistant (Wyoming bridge removed in 0.4.0)](HOMEASSISTANT.md)

## Architecture, privacy, and security

- [Canonical architecture](../ARCHITECTURE.md)
- [Architecture guide and related design material](ARCHITECTURE.md)
- [Agent protocol v1](../protocols/agent.md)
- [Adapter contract](../adapters/CONTRACT.md)
- [Session model](../gateway/SESSION_MODEL.md)
- [Privacy](PRIVACY.md)
- [Security policy](../SECURITY.md)
- [Supply chain](SUPPLY_CHAIN.md)
- [Namespace migration ADR](architecture/NAMESPACE_MIGRATION_ADR.md)
- [Turn lifecycle hardening plan](architecture/TURN_LIFECYCLE_HARDENING_PLAN.md)
- [Stream decoder hardening plan](architecture/STREAM_DECODER_HARDENING_PLAN.md)

## Contributors and maintainers

- [Documentation governance](DOCUMENTATION_GOVERNANCE.md)
- [Contributing](../CONTRIBUTING.md)
- [Developer onboarding](DEVELOPER_ONBOARDING.md)
- [Governance](../GOVERNANCE.md)
- [Support](../SUPPORT.md)
- [Good first issue candidates](GOOD_FIRST_ISSUES.md)
- [Benchmarks and regression budgets](BENCHMARKS.md)
- [Demo runbook](DEMO.md)
- [Demo asset plan](DEMO_PLAN.md)
- [Marketing copy](MARKETING.md)
- [Release process](RELEASE_PROCESS.md)
- [Release checklist](RELEASE_CHECKLIST.md)
- [Release evidence format](releases/README.md)
- [Roadmap](../ROADMAP.md)
- [Changelog](../CHANGELOG.md)

Maintainer material is not a product contract. Revalidate demo, marketing, issue, and release-checklist claims against the feature matrix and current implementation before reuse.

## Versioned release notes

- [Qantara 0.4.0](RELEASE_NOTES_0.4.0.md)
- [Qantara 0.3.1](RELEASE_NOTES_0.3.1.md)
- [Qantara 0.3.0](RELEASE_NOTES_0.3.0.md)
- [Qantara 0.2.12](RELEASE_NOTES_0.2.12.md)

The tagged GitHub Release and attached validation evidence are authoritative for published artifacts.

## Historical snapshots

These files preserve a dated decision/audit state and are not current product guidance:

- [First public release notes draft](FIRST_PUBLIC_RELEASE_NOTES_DRAFT.md)
- [Publishing readiness audit](PUBLISHING_READINESS_AUDIT.md)
- [Repository cleanup report](REPOSITORY_CLEANUP_REPORT.md)
- [Security publication audit](SECURITY_PUBLICATION_AUDIT.md)
- [Session handoff](SESSION_HANDOFF.md)

Every historical snapshot carries a warning at the top. Open items or scores in those files must not be treated as current backlog or current readiness.
