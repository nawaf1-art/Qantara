"""Qantara CLI — single-command launcher for the voice gateway.

Usage examples:
    qantara --backend mock
    qantara --backend ollama --model qwen3.5:2b
    qantara --backend openclaw --agent main
    qantara --backend http://my-service:8080
    qantara doctor [--mesh]

From a source checkout, ``python cli.py ...`` is equivalent.

Startup precedence (highest wins):
explicit CLI flags > environment variables > selected YAML file > built-in defaults.
"""

from __future__ import annotations

import argparse
import asyncio
import os
import signal
import sys
import time
from pathlib import Path

from qantara.config import DEFAULTS, ConfigError, env_int, load_config
from qantara.security import bridge_subprocess_environment, sanitize_public_url

# The installed package root (site-packages for a wheel, the repository root
# for a source checkout); gateway/ and friends live next to qantara/.
PACKAGE_ROOT = Path(__file__).resolve().parent.parent
REPO_ROOT = str(PACKAGE_ROOT)

_BRIDGE_SCRIPTS: dict[str, str] = {
    "ollama": str(PACKAGE_ROOT / "gateway" / "ollama_session_backend" / "server.py"),
    "openclaw": str(PACKAGE_ROOT / "gateway" / "openclaw_session_backend" / "server.py"),
}

MANAGED_BRIDGE_PORT = 19120

# (argument name, environment variable, config section, config key, default)
_STARTUP_FIELDS: tuple[tuple[str, str, str, str, str], ...] = (
    ("backend", "QANTARA_BACKEND", "backend", "type", ""),
    ("model", "QANTARA_OLLAMA_MODEL", "backend", "model", ""),
    ("agent", "QANTARA_OPENCLAW_AGENT_ID", "backend", "agent", ""),
    ("host", "QANTARA_SPIKE_HOST", "server", "host", "127.0.0.1"),
)


# ---------------------------------------------------------------------------
# Argument parsing
# ---------------------------------------------------------------------------
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="qantara",
        description=(
            "Start the Qantara voice gateway with a single command. "
            "Run `qantara doctor` to check the local environment."
        ),
        epilog=(
            "Precedence: explicit CLI flags > environment variables > "
            "selected YAML file > built-in defaults."
        ),
    )
    parser.add_argument(
        "--backend",
        default=None,
        help="Backend: mock, ollama, openclaw, openai_compatible, or an HTTP(S) URL (env: QANTARA_BACKEND).",
    )
    parser.add_argument(
        "--model",
        default=None,
        help="Model name for the Ollama or OpenAI-compatible backend (env: QANTARA_OLLAMA_MODEL).",
    )
    parser.add_argument(
        "--agent",
        default=None,
        help="Agent ID for the advanced OpenClaw bridge (env: QANTARA_OPENCLAW_AGENT_ID).",
    )
    parser.add_argument("--host", default=None, help="Gateway bind host (env: QANTARA_SPIKE_HOST).")
    parser.add_argument("--port", type=int, default=None, help="Gateway TCP port (env: QANTARA_SPIKE_PORT).")
    parser.add_argument(
        "--config",
        default=None,
        help="Path to a qantara.yml file; it must exist (env: QANTARA_CONFIG).",
    )
    return parser


def _apply_config_defaults(args: argparse.Namespace) -> None:
    """Resolve every startup value: flag > env > config file > default.

    ``args.<field>`` is None when the flag was not given. Records where each
    value came from in ``args._sources`` so later steps know whether an
    explicit flag must override lower-level environment variables.
    """
    cfg = load_config(args.config or None, include_defaults=False)
    sources: dict[str, str] = {}

    for name, env_name, section, key, default in _STARTUP_FIELDS:
        if getattr(args, name) is not None:
            sources[name] = "flag"
            continue
        env_value = os.environ.get(env_name, "").strip()
        if env_value:
            setattr(args, name, env_value)
            sources[name] = "env"
        elif cfg[section][key]:
            setattr(args, name, cfg[section][key])
            sources[name] = "config"
        else:
            setattr(args, name, default)
            sources[name] = "default"

    if args.port is not None:
        sources["port"] = "flag"
    elif os.environ.get("QANTARA_SPIKE_PORT", "").strip():
        args.port = env_int("QANTARA_SPIKE_PORT", 8765, minimum=1, maximum=65535)
        sources["port"] = "env"
    elif cfg["server"]["port"]:
        args.port = int(cfg["server"]["port"])
        sources["port"] = "config"
    else:
        args.port = int(DEFAULTS["server"]["port"])
        sources["port"] = "default"
    if not 0 < args.port < 65536:
        raise ConfigError(f"--port must be between 1 and 65535, got {args.port}")

    # Voice providers have no flags: the YAML value applies unless env is set.
    if cfg["voice"]["stt"] and not os.environ.get("QANTARA_STT_PROVIDER"):
        os.environ["QANTARA_STT_PROVIDER"] = cfg["voice"]["stt"]
    if cfg["voice"]["tts"] and not os.environ.get("QANTARA_TTS_PROVIDER"):
        os.environ["QANTARA_TTS_PROVIDER"] = cfg["voice"]["tts"]

    # backend.url is config-level: adapter-specific URL env vars still win.
    args._config_backend_url = cfg["backend"]["url"]
    args._sources = sources



# ---------------------------------------------------------------------------
# Backend classification
# ---------------------------------------------------------------------------
def _classify_backend(backend: str) -> tuple[str, str]:
    """Return (backend_type, url) from the --backend value.

    backend_type is one of: mock, ollama, openclaw, openai_compatible, custom.
    url is the backend URL (empty for mock, ollama, openclaw unless overridden).
    """
    value = backend.strip()
    lower = value.lower()

    if not lower or lower == "mock":
        return "mock", ""
    if lower == "ollama":
        return "ollama", ""
    if lower == "openclaw":
        return "openclaw", ""
    if lower in ("openai", "openai_compatible", "openai-compatible"):
        return "openai_compatible", ""
    if lower.startswith("http://") or lower.startswith("https://"):
        # HTTP URLs → OpenAI-compatible adapter (most common use case)
        return "openai_compatible", value.rstrip("/")

    # Unrecognised — treat as literal and let the user know
    print(
        "[qantara] warning: unrecognised backend value; treating it as a custom URL",
        flush=True,
    )
    return "custom", value


# ---------------------------------------------------------------------------
# Environment setup — sets env vars before gateway module is imported
# ---------------------------------------------------------------------------
def _set(name: str, value: str, *, override: bool) -> None:
    """Set an adapter variable; an explicit flag overrides an inherited env value."""
    if not value:
        return
    if override or not os.environ.get(name):
        os.environ[name] = value


def _apply_env(backend_type: str, url: str, args: argparse.Namespace) -> None:
    """Set QANTARA_* environment variables so the gateway picks up the config."""
    sources: dict[str, str] = getattr(args, "_sources", {})
    backend_from_flag = sources.get("backend") == "flag"
    model_from_flag = sources.get("model") == "flag"
    agent_from_flag = sources.get("agent") == "flag"
    config_url = getattr(args, "_config_backend_url", "")
    managed_bridge_url = f"http://127.0.0.1:{MANAGED_BRIDGE_PORT}"

    if backend_type == "mock":
        os.environ["QANTARA_ADAPTER"] = "mock"

    elif backend_type == "ollama":
        os.environ["QANTARA_ADAPTER"] = "session_gateway_http"
        _set("QANTARA_BACKEND_BASE_URL", managed_bridge_url, override=backend_from_flag)
        _set("QANTARA_OLLAMA_MODEL", args.model, override=model_from_flag)
        # backend.url from YAML is the Ollama upstream for the bridge.
        _set("QANTARA_OLLAMA_BASE_URL", config_url, override=False)

    elif backend_type == "openclaw":
        os.environ["QANTARA_ADAPTER"] = "session_gateway_http"
        _set("QANTARA_BACKEND_BASE_URL", managed_bridge_url, override=backend_from_flag)
        _set("QANTARA_OPENCLAW_AGENT_ID", args.agent, override=agent_from_flag)

    elif backend_type == "openai_compatible":
        os.environ["QANTARA_ADAPTER"] = "openai_compatible"
        if url:
            _set("QANTARA_OPENAI_BASE_URL", url, override=backend_from_flag)
        else:
            _set("QANTARA_OPENAI_BASE_URL", config_url, override=False)
        _set("QANTARA_OPENAI_MODEL", args.model, override=model_from_flag)

    elif backend_type == "custom":
        os.environ["QANTARA_ADAPTER"] = "session_gateway_http"
        if url:
            _set("QANTARA_BACKEND_BASE_URL", url, override=backend_from_flag)
        else:
            _set("QANTARA_BACKEND_BASE_URL", config_url, override=False)

    # Gateway host/port are already resolved with full precedence.
    os.environ["QANTARA_SPIKE_HOST"] = args.host
    os.environ["QANTARA_SPIKE_PORT"] = str(args.port)


# ---------------------------------------------------------------------------
# Bridge subprocess management
# ---------------------------------------------------------------------------
_bridge_proc: asyncio.subprocess.Process | None = None


async def _start_bridge(backend_type: str, args: argparse.Namespace) -> None:
    """Start the appropriate bridge subprocess for ollama/openclaw."""
    global _bridge_proc

    script = _BRIDGE_SCRIPTS.get(backend_type)
    if script is None or not os.path.isfile(script):
        print(f"[qantara] error: bridge script not found for '{backend_type}'", flush=True)
        sys.exit(1)

    env = bridge_subprocess_environment()
    env["QANTARA_REAL_BACKEND_PORT"] = str(MANAGED_BRIDGE_PORT)
    env["QANTARA_REAL_BACKEND_HOST"] = "127.0.0.1"
    if backend_type == "ollama" and os.environ.get("QANTARA_OLLAMA_MODEL"):
        env["QANTARA_OLLAMA_MODEL"] = os.environ["QANTARA_OLLAMA_MODEL"]
    if backend_type == "openclaw" and os.environ.get("QANTARA_OPENCLAW_AGENT_ID"):
        env["QANTARA_OPENCLAW_AGENT_ID"] = os.environ["QANTARA_OPENCLAW_AGENT_ID"]

    print(f"[qantara] starting {backend_type} bridge on port {MANAGED_BRIDGE_PORT}...", flush=True)
    bridge_logs_enabled = os.environ.get("QANTARA_BRIDGE_LOG_OUTPUT", "").strip().lower() in {
        "1",
        "true",
        "yes",
    }
    _bridge_proc = await asyncio.create_subprocess_exec(
        sys.executable, script,
        env=env,
        stdout=None if bridge_logs_enabled else asyncio.subprocess.DEVNULL,
        stderr=None if bridge_logs_enabled else asyncio.subprocess.DEVNULL,
    )


async def _wait_for_bridge(timeout: float = 8.0) -> bool:
    """Poll bridge health endpoint until ready or timeout."""
    import aiohttp

    url = f"http://127.0.0.1:{MANAGED_BRIDGE_PORT}/health"
    deadline = time.monotonic() + timeout
    client_timeout = aiohttp.ClientTimeout(total=2)
    while time.monotonic() < deadline:
        try:
            async with aiohttp.ClientSession(timeout=client_timeout, trust_env=False) as session:
                async with session.get(url, allow_redirects=False) as resp:
                    if resp.status < 500:
                        return True
        except Exception:
            pass
        await asyncio.sleep(0.4)
    return False


async def _stop_bridge() -> None:
    """Gracefully stop the bridge subprocess."""
    global _bridge_proc
    if _bridge_proc is None:
        return
    try:
        _bridge_proc.terminate()
        try:
            await asyncio.wait_for(_bridge_proc.wait(), timeout=5)
        except TimeoutError:
            _bridge_proc.kill()
            await _bridge_proc.wait()
    except ProcessLookupError:
        pass
    _bridge_proc = None


# ---------------------------------------------------------------------------
# Main entry point
# ---------------------------------------------------------------------------
async def _run(args: argparse.Namespace) -> None:
    backend_type, url = _classify_backend(args.backend)

    # Apply env vars before importing the gateway module (it reads env at import time)
    _apply_env(backend_type, url, args)
    runner = None
    try:
        # Start bridge subprocess if needed
        if backend_type in ("ollama", "openclaw"):
            await _start_bridge(backend_type, args)
            ready = await _wait_for_bridge()
            if not ready:
                print(f"[qantara] warning: {backend_type} bridge did not become healthy in time", flush=True)
                print("[qantara] continuing anyway — the bridge may still be loading...", flush=True)

        # Now import the gateway — it reads QANTARA_* env vars at module level
        from gateway.transport_spike.server import create_app, create_ssl_context

        label = backend_type
        if backend_type == "ollama" and args.model:
            label = f"ollama ({args.model})"
        elif backend_type == "openclaw" and args.agent:
            label = f"openclaw ({args.agent})"
        elif backend_type == "custom":
            display_url = sanitize_public_url(
                url if url.startswith(("http://", "https://")) else f"http://{url}"
            )
            label = f"custom ({display_url or 'configured endpoint'})"

        print(f"[qantara] backend: {label}", flush=True)
        print(f"[qantara] gateway: http://{args.host}:{args.port}", flush=True)

        from aiohttp import web

        app = create_app()
        runner = web.AppRunner(app)
        await runner.setup()
        site = web.TCPSite(runner, args.host, args.port, ssl_context=create_ssl_context())
        await site.start()

        stop_event = asyncio.Event()
        loop = asyncio.get_running_loop()
        for sig in (signal.SIGINT, signal.SIGTERM):
            try:
                loop.add_signal_handler(sig, stop_event.set)
            except NotImplementedError:
                # Windows delivers Ctrl+C as KeyboardInterrupt. The finally
                # block below still closes the runner and managed bridge.
                pass
        await stop_event.wait()
    finally:
        print("\n[qantara] shutting down...", flush=True)
        if runner is not None:
            await runner.cleanup()
        await _stop_bridge()
        print("[qantara] stopped.", flush=True)


def main(argv: list[str] | None = None) -> int:
    argv = list(sys.argv[1:] if argv is None else argv)
    if argv[:1] == ["doctor"]:
        from qantara.doctor import main as doctor_main

        return doctor_main(argv[1:])

    parser = build_parser()
    args = parser.parse_args(argv)
    try:
        _apply_config_defaults(args)
    except ConfigError as exc:
        print(f"[qantara] error: {exc}", file=sys.stderr, flush=True)
        return 2

    if not args.backend:
        args.backend = "mock"

    try:
        asyncio.run(_run(args))
    except KeyboardInterrupt:
        pass
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
