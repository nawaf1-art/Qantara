...
class UrlSafetyTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_allows_loopback_and_rfc1918(self) -> None:
        self.assertTrue(await is_safe_url("http://127.0.0.1:11434"))
        self.assertTrue(await is_safe_url("http://192.168.1.10:8080"))
        self.assertTrue(await is_safe_url("http://10.0.0.5"))
        self.assertTrue(await is_safe_url("http://172.16.3.4:1234"))
        self.assertTrue(await is_safe_url("http://[::1]:8000"))

    async def test_rejects_public(self) -> None:
        self.assertFalse(await is_safe_url("http://8.8.8.8"))
        self.assertFalse(await is_safe_url("https://example.com"))

    async def test_rejects_embedded_credentials(self) -> None:
        self.assertFalse(await is_safe_url("http://user:password@127.0.0.1:11434"))
        self.assertFalse(await is_safe_url("http://token@192.168.1.10:8080"))

    async def test_rejects_link_local_cloud_metadata(self) -> None:
        ...
        self.assertFalse(await is_safe_url("http://169.254.1.1:80"))

    async def test_rejects_unspecified(self) -> None:
        # 0.0.0.0 / :: are is_private == True but route to localhost on Linux.
        self.assertFalse(await is_safe_url("http://0.0.0.0:8080"))
        self.assertFalse(await is_safe_url("http://[::]:8080"))

    async def test_rejects_ipv4_mapped_link_local(self) -> None:
        # ::ffff:169.254.169.254 must not smuggle the metadata IP past the guard.
        self.assertFalse(await is_safe_url("http://[::ffff:169.254.169.254]"))

    async def test_rejects_ipv6_link_local(self) -> None:
        self.assertFalse(await is_safe_url("http://[fe80::1]"))

    async def test_hostname_pin_prefers_private_ipv4_for_dual_stack_backends(self) -> None:
        addresses = [
            (10, 1, 6, "", ("::1", 11434, 0, 0)),
        ...
        with patch(
            ...
            target = await _safe_outbound_url("http://localhost:11434")
